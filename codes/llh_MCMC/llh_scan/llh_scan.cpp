...
for (unsigned int i = 1; i < count+1; i++) {
  mu = i/100.0;
  double calc = f_gllh(mu,sigma,gauss_points);
  mu_p[i] = mu;
...
for (unsigned int i = 15; i < count+15; i++) {
  sigma = i/100.0;
  double calc = f_gllh(mu,sigma,gauss_points);
  sigma_p[i] = sigma;
...
TGraph * gr2 = new TGraph(count,sigma_p,llh_p2);
c1.cd();
gr->Draw("A*");
